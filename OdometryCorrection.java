...
public class OdometryCorrection extends Thread {
	...
	// Runs Odometry Correction. Uses the light sensor to correct the robot's position.
	public void run() {
		//Fetches the sample from the light sensor and saves it into a set light value
		colorValue.fetchSample(colorData, 0);
		double colourLast = colorData[0];
		double pos[] = new double[3];
		while(true){
			//A new value that is compared to the value from before to see if a blackline was hit.
			colorValue.fetchSample(colorData, 0);
			...
			if(colourLast-colourNow>0.1){
				int i = 0;
				int j = 0;
				//Subtract the position of x/y the tile-spacing until it achieves >-tile-spacing.
				//The purpose of this is to see if the expected value of x/y and the actual value of x/y
				//are in sync. A counter is then increased for every time tile-spacing is subtracted
				...
				//is expected to be close to an intersection, ignore the value given
				if(setX && setY){
				}
